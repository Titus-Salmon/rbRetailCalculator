...
module.exports = {
  ...
  retailAuditInvestigation: router.post('/search', (req, res, next) => {

    // let retailDifferenceArr = []
    // console.log('retailAuditInvestigation.js says retailDifferenceArr==>', retailDifferenceArr)
    ...
    let upcQueryArr = postBody['upcQueryArr']
    ...
    connection.query(
      ...
      `SELECT DISTINCT * FROM ${retailDiffTableOld} WHERE rb_upc IN (${upcQueryArr});`,
      function (err, rows, fields) {
        if (err) throw err

        retailAudResults(rows)

        res.render('vw-retailAuditInvestigation', { //render searchResults to vw-retailCalcPassport page
