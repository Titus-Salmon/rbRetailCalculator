...
const {
  searchEditCalcTableTableJoin
} = require('../sqlArch/searchEditCalcTableTableJoin')

const {
  loadTableTableJoin
} = require('../sqlArch/loadTableTableJoin')
...
router.get('/', ensureAuthenticated, function (req, res, next) {
  res.render('vw-retailCalcTableJoin', {
    title: 'Retail Price Calculator (table join method)',
    username: req.user.name,
    userEmail: req.user.email,
    userEmail_stringified: JSON.stringify(req.user.email),
    // currentTableName_stringified: JSON.stringify(loadedTable.tableNameToLoad),
  });
  // console.log('req from /retailCalcTableJoin==========>', req)
});

router.post('/createTable', createTable)
router.post('/populateTable', populateTable)
router.post('/results', searchEditCalcTableTableJoin)
router.post('/loadTableTableJoin', loadTableTableJoin)
